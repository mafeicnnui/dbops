...
class get_sync_park_real_time(basehandler):
    def post(self):
        self.set_header("Content-Type", "application/json; charset=UTF-8")
        v_list     = query_sync_park_real_time()
        v_json     = json.dumps(v_list)
        self.write(v_json)
...
class get_sync_flow_real_time(basehandler):
    def post(self):
        self.set_header("Content-Type", "application/json; charset=UTF-8")
        v_list     = query_sync_flow_real_time()
        v_json     = json.dumps(v_list)
        self.write(v_json)
...
class sys_stats_num(basehandler):
    def post(self):
        ...
        begin_date = self.get_argument("begin_date")
        end_date   = self.get_argument("end_date")
        v_list     = query_sys_stats_num(server_id,begin_date,end_date)
        v_json     = json.dumps(v_list)
        self.write(v_json)
