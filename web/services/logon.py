...
from web.model.t_user import logon_user_check, check_forget_password, check_modify_password, \
    save_forget_authention_string, check_auth_str_exist
...
class forget_password_check_user(base_handler.BaseHandler):
    async def post(self):
        ...
        if result['code'] == '0':
            ...
            v_content = """<p><h4>用户名：</h4>{}<p><h4>授权码：</h4>{}<p><h4>有效期：</h4>1分钟""".format(user,
                                                                                                        auth_string)
