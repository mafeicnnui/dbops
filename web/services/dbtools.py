...
from web.model.t_dmmx import get_sync_db_mysql_server, get_compare_db_server, \
    get_dmm_from_dm_cipher
...
class _db_compare_detail_idx(base_handler.TokenHandler):
    async def post(self):
        ...
        v_list = await db_stru_compare_detail_idx(sour_db_server, sour_schema, desc_db_server, desc_schema, table,
                                                  index)
        self.write({"code": 0, "message": v_list})
...
class _db_compare_statement(base_handler.TokenHandler):
    async def post(self):
        ...
        table = self.get_argument("table")
        column = self.get_argument("column")
        v_list = await db_stru_compare_statement(sour_db_server, sour_schema, desc_db_server, desc_schema, table,
                                                 column)
        self.write({"code": 0, "message": v_list})
