...
from web.model.t_dmmx import get_dmm_from_dm, get_gather_server, get_templete_names, get_sync_db_server, \
    get_gather_tasks, get_db_moitor_templates
from web.model.t_ds import get_dss
from web.model.t_monitor import get_monitor_indexes, get_monitor_indexes2, get_monitor_indexes_by_type, \
    get_monitor_task_by_tag, query_monitor_sys, query_power_empty, query_power_full, query_power_offline
from web.model.t_monitor import get_monitor_sys_indexes, get_monitor_templete_indexes, save_gather_task, \
    save_monitor_task, query_task
from web.model.t_monitor import push_monitor_task, run_monitor_task, stop_monitor_task, query_monitor_svr, \
    query_monitor_proj, query_monitor_proj_log
...
from web.model.t_monitor import query_monitor_templete, save_templete, upd_templete, del_templete, del_task, \
    upd_gather_task, upd_monitor_task
from web.utils import base_handler
from web.utils.common import get_day_nday_ago, now, DateEncoder
...
class monitor_power_empty_warn(base_handler.BaseHandler):
    async def post(self):
        v_list = await query_power_empty()
        v_json = json.dumps(v_list)
        print(v_json)
        self.write(v_json)


class monitor_power_empty(base_handler.BaseHandler):
    async def get(self):
        self.render("./monitor/monitor_power_empty.html")


class monitor_power_full_warn(base_handler.BaseHandler):
    async def post(self):
        v_list = await query_power_full()
        v_json = json.dumps(v_list)
        print(v_json)
        self.write(v_json)
