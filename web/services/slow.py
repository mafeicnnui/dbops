...
from web.model.t_dmmx import get_dmm_from_dm, get_slow_inst_names, get_slow_dbs_names, get_gather_server, \
    get_sync_db_server
from web.model.t_ds import get_ds_by_dsid
from web.model.t_slow import push_slow, query_slow_log_by_id, query_slow_log_by_id_oracle, query_slow_log_by_id_mssql, \
    query_slow_log, query_slow_log_detail, \
...
class slowlog_query(base_handler.TokenHandler):
    async def post(self):
        ...
        if dss['db_type'] == '1':
            v_list = await query_slow_log_oracle(ds_id, db_user, begin_date, end_date, begin_query_time, end_query_time,
                                                 db_sql)
        elif dss['db_type'] == '2':
            v_list = await query_slow_log_mssql(ds_id, db_user, begin_date, end_date, begin_query_time, end_query_time,
                                                db_sql)
        else:
            v_list = await query_slow_log(inst_id, ds_id, db_name, db_user, db_host, begin_date, end_date,
                                          begin_query_time, end_query_time, db_sql)
        v_json = json.dumps(v_list)
        self.write(v_json)
...
class slowlog_analyze(base_handler.TokenHandler):
    async def post(self):
        ...
        end_query_time = self.get_argument("end_query_time")
        db_sql = self.get_argument("db_sql")
        v_list = await analyze_slow_log(inst_id, ds_id, db_name, db_user, db_host, begin_date, end_date,
                                        begin_query_time, end_query_time, db_sql)
        v_json = json.dumps(v_list)
        self.write(v_json)
