...
from web.utils.common import get_connection_ds_pg, get_connection_ds_mongo, get_connection_ds_redis, \
    get_connection_ds_es
    ...
    if p_name == "":
        sql = """SELECT  a.id,
                ...
                WHERE a.db_type=b.dmm AND b.dm='02'
                  ...
                  and a.status='1'
                  {6}
                ORDER BY a.ip,PORT,a.service""".format(p_userid, p_userid, p_userid, p_userid, p_userid, p_userid,
                                                       v_where)
    else:
        sql = """SELECT a.id,
                        a.db_desc,
    ...
    try:
        if p_env in ('1'):
            for plain in p_plain.split(','):
                ...
                rs = await async_processer.query_dict_one(
                    'select pro_cipher,pro_like from t_cipher where id={}'.format(p_userid))
                res = res + '-' * 20 + plain + '-' * 20 + '\ncipher列密文：\n  ' + rs[
                    'pro_cipher'] + '\nlike列密文：\n  ' + rs['pro_like'] + '\n\n'
            ...
            for plain in p_plain.split(','):
                ...
                await async_processer.exec_sql_by_ds(ss_agent_ds, st)
                rs = await async_processer.query_dict_one(
                    'select dev_cipher,dev_like from t_cipher where id={}'.format(p_userid))
                res = res + '-' * 20 + plain + '-' * 20 + '\ncipher列密文：\n  ' + rs['dev_cipher'] + '\nLIKE列密文:' + \
                      rs['dev_like'] + '\n\n'
            return {"code": 0, "message": res[0:-1]}
    except:
...
def get_ciphers(p_cipher):
    res = []
    for cipher in p_cipher.split('\n\n'):
        res.append(cipher.split('\n')[2].strip())
    return ','.join(res)
...
async def db_decrypt(p_env, p_cipher, p_userid):
    ...
    try:
        if p_env in ('1'):
            for cipher in get_ciphers(p_cipher).split(','):
                st = """UPDATE t_cipher SET pro_cipher='{}' WHERE id={}""".format(cipher, p_userid)
                await async_processer.exec_sql_by_ds(ss_agent_ds, st)
                rs = await async_processer.query_dict_one_by_ds(ss_agent_ds,
                                                                'select pro from t_cipher where id={}'.format(p_userid))
                res = res + rs['pro'] + ','
            return {"code": 0, "message": res[0:-1]}
        else:
            for cipher in get_ciphers(p_cipher).split(','):
                ...
                await async_processer.exec_sql_by_ds(ss_agent_ds, st)
                rs = await async_processer.query_dict_one_by_ds(ss_agent_ds,
                                                                'select dev from t_cipher where id={}'.format(p_userid))
                res = res + rs['dev'] + ','
            return {"code": 0, "message": res[0:-1]}
    except:
        traceback.print_exc()
