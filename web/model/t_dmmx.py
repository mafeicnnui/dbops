...
async def get_users(p_username):
    if p_username == 'admin':
        ...
        sql = """select login_name,concat(name,'(',wkno,')') from t_user where login_name='{}' order by id """.format(
            p_username)
    return await async_processer.query_list(sql)
...
async def get_users_by_query_grants(p_username):
    if p_username == 'admin':
        ...
        sql = """select id,concat(name,'(',wkno,')') from t_user where login_name='{}' order by id """.format(
            p_username)
    return await async_processer.query_list(sql)
...
async def get_slow_dbs_names(p_env):
    if p_env == '':
        ...
        sql = "SELECT a.ds_id,b.db_desc FROM t_slow_log a,t_db_source b WHERE  a.ds_id=b.id AND a.STATUS='1' and b.db_env='{}' order by b.db_desc".format(
            p_env)
    return await async_processer.query_list(sql)
...
async def get_slow_inst_names(p_env):
    if p_env == '':
        ...
        sql = "SELECT a.inst_id, b.inst_name FROM t_slow_log a,t_db_inst b WHERE  a.inst_id=b.id AND a.STATUS='1'  and b.inst_env='{}'  order by b.inst_name".format(
            p_env)
    return await async_processer.query_list(sql)
...
async def get_bbtj_db_imp_server():
    sql = """SELECT id,db_desc FROM t_db_source 
              WHERE  id in(16,19,84) ORDER BY db_desc,db_type"""
    return await async_processer.query_list(sql)
