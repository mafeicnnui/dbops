...
async def save_port(p_port):
    ...
    try:
        ...
        sql = """insert into t_port(market_id,market_name,app_desc,local_ip,local_port,mapping_port,mapping_domain,mapping_type,creater,create_date) 
                 values('{}','{}','{}','{}','{}','{}','{}','{}','{}',now())
            """.format(market_id, market_name, app_desc, local_ip, local_port, mapping_port, mapping_domain,
                       mapping_type, creater)
    ...
    try:
        ...
        sql = """update t_port
                  set  
                      ...
                      update_date     = now()            
                where id={}""". \
            format(market_id, market_name, app_desc, local_ip, local_port, mapping_port, mapping_domain, mapping_type,
                   port_id)
        await async_processer.exec_sql(sql)
        result = {}
        result['code'] = '0'
...
async def get_port_by_portid(p_portid):
    sql = """select  id,market_id,app_desc,local_ip,local_port,mapping_port,mapping_domain,mapping_type from t_port where id={0}""".format(
        p_portid)
...
async def imp_port(p_file, p_name):
    try:
        ...
        sql = "insert into t_port(market_id,market_name,app_desc,local_ip,local_port,mapping_port,mapping_domain) values {0}".format(
            vals[0:-1])
        await async_processer.exec_sql(sql)
