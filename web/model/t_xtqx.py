...
async def get_menu_by_menuid(p_menuid):
    sql = "select id as menuid,name,status,url,parent_id,creation_date,creator,last_update_date,updator from t_xtqx where id={0}".format(
        p_menuid)
...
async def get_menuid(p_parent_id):
    sql = "SELECT count(0),CAST(CONCAT('0',MAX(id)+1) AS CHAR) AS ID FROM t_xtqx WHERE parent_id='{0}'".format(
        p_parent_id)
    rs = await async_processer.query_one(sql)
    if rs[0] == 0:
...
async def save_func(p_func):
    ...
    try:
        sql = """insert into t_func(func_name,func_url,priv_id,status,creation_date,creator,last_update_date,updator) 
            ...
            """.format(p_func['func_name'], p_func['func_url'], p_func['priv_id'], p_func['status'], current_rq(),
                       'DBA', current_rq(), 'DBA')
        await async_processer.exec_sql(sql)
        res = {}
        res['code'] = '0'
    ...
    try:
        ...
        for i in range(len(rs1)):
            ...
            for j in range(len(rs2)):
                v_node = """<li><a id="{0}^{1}" class="file" href="javascript:void(0);">{2}</a></li>""".format(
                    rs2[j][2][1:].replace('/', '_'), rs2[j][2], rs2[j][1])
                v_html = v_html + "\n" + v_node;
            v_html = v_html + "\n" + v_menu_footer + "\n"
        result['code'] = '0'
...
async def get_tree_by_dbid_mongo(dbid):
    try:
        result = {}
        p_ds = await get_ds_by_dsid(dbid)
        mongo = mongo_client(p_ds['ip'], p_ds['port'], auth_db='admin', db='admin', user=p_ds['user'],
...
async def get_tree_by_dbid_proxy_query_grants(dbid, msg, userid):
    try:
        ...
        for db in rs1:
            ...
            rs3 = await async_processer.query_dict_one(sql3.format(dbid, userid, db['schema_name']))
            ret2 = get_mysql_proxy_result_dict(p_ds, sql2.format(db['schema_name'], rs3['query_table']),
                                               p_ds['service'])
            if ret1['status'] == '1':
                result['code'] = '-1'
                result['message'] = '加载失败！'
