...
def get_hbase_rowkey(p_sync):
    ...
    for idx in range(len(p_sync['sync_hbase_rowkey'].split(','))):
        print('get_hbase_rowkey=', idx, p_sync['sync_hbase_rowkey_separator'],
              len(p_sync['sync_hbase_rowkey'].split(',')))
...
async def process_templete(p_sync_id, p_templete):
    ...
    v_templete['full'] = v_templete['full'].replace('$$PASSWORD$$',
                                                    await aes_decrypt(p_sync['password'], p_sync['user']))
    ...
    v_templete['incr'] = v_templete['incr'].replace('$$PASSWORD$$',
                                                    await aes_decrypt(p_sync['password'], p_sync['user']))
...
async def process_templete_doris(p_sync_id, p_templete):
    ...
    v_templete['full'] = v_templete['full'].replace('$$DORIS_PASSWORD$$',
                                                    await aes_decrypt(p_sync['doris_password'], p_sync['doris_user']))
    ...
    v_templete['incr'] = v_templete['incr'].replace('$$DORIS_PASSWORD$$',
                                                    await aes_decrypt(p_sync['doris_password'], p_sync['doris_user']))
...
async def check_tab_exists_pk(cfg):
    ...
    st = """select count(0) from information_schema.columns
              where table_schema='{}' and table_name='{}' and column_key='PRI'""".format(cfg['sour_db_name'],
                                                                                         cfg['doris_tab_name'])
    ...
    try:
        ...
        sql = """insert into t_datax_sync_config(
            ...
            """.format(sync_tag, sync_server, sour_db_server, sour_db_name, sour_tab_name,
                       ...
                       sync_gap, api_server, status, sync_hbase_table, sync_hbase_rowkey,
                       sync_hbase_rowkey_separator, sync_hbase_columns, sync_hbase_rowkey_sour, sync_incr_where,
                       python3_home,
                       hbase_thrift, es_service, es_index_name, es_type_name, sync_es_columns,
...
async def query_sync_log_analyze(sync_env, tagname, begin_date, end_date):
    ...
    if end_date != '':
        v_where = v_where + " and a.create_date<='{0}'\n".format(end_date + ' 23:59:59')
    sql1 = """SELECT cast(a.create_date as char) as create_date,a.duration FROM t_datax_sync_log a {0} ORDER BY a.create_date""".format(
        v_where)
    sql2 = """SELECT cast(a.create_date as char) as create_date,a.amount  FROM t_datax_sync_log a {0} ORDER BY a.create_date""".format(
        v_where)
...
async def get_datax_sync_tags_by_env(p_env):
    ...
    if p_env == '':
        ...
        sql = """SELECT a.sync_tag,a.comments FROM t_datax_sync_config a  WHERE STATUS=1 {0}  ORDER BY comments""".format(
            v_where)
