    ...
    if dres is None:
        st = """ALTER TABLE `{}`.`{}` ADD `{}` {} {} {} {} {} {} {};
             """.format(desc_schema,
                        ...
                        sres['column_name'],
                        sres['column_type'],
                        ' CHARSET ' + sres['character_set_name'] if sres['character_set_name'] is not None and sres[
                            'character_set_name'] != '' else '',
                        'COLLATE ' + sres['collation_name'] if sres['collation_name'] is not None and sres[
                            'collation_name'] != '' else '',
                        'DEFAULT ' + sres['column_default'] if sres['column_default'] is not None and sres[
                            'column_default'] != '' else '',
                        sres['extra'] if sres['extra'] is not None and sres['extra'] != '' else '',
                        'NULL ' if sres['is_nullable'] == 'YES' else 'NOT NULL',
                        "COMMENT '" + sres['column_comment'] + "'" if sres['column_comment'] is not None and sres[
                            'column_comment'] != '' else '')
        ...
        st = """ALTER TABLE `{}`.`{}` CHANGE `{}` `{}` {} {} {} {} {} {} {};
             """.format(dres['table_schema'],
                        ...
                        ' CHARSET ' + sres['character_set_name']
                                     if sres['character_set_name'] is not None and sres[
                            'character_set_name'] != '' else '',
    ...
    for r in res:
        ...
        for k, v in sres.items():
            if dres is None:
                ...
                await async_processer.exec_sql(
                    """insert into t_db_compare_detail(property_name,sour_property_value,desc_property_value,status,statement)
                       values('{}','{}','{}','{}','{}')""".format(k, format_sql(sres[k]), format_sql(dres[k]), '1',
                                                                  format_sql(get_sync_sql(sres, dres))))
                break
            else:
                await async_processer.exec_sql(
            ...
            if dres is None:
                await async_processer.exec_sql(
                    """insert into t_db_compare_detail(property_name,sour_property_value,desc_property_value,status,statement)
                       values('{}','{}','{}','{}','{}')""".format(k, sres[k], '', '1',
                                                                  format_sql(
                                                                      get_sync_sql_idx(sres, dres, desc_schema))))
                break
            elif sres[k] != dres[k]:
                await async_processer.exec_sql(
                    """insert into t_db_compare_detail(property_name,sour_property_value,desc_property_value,status,statement)
...
async def db_stru_compare_ck_data(sour_db_server, sour_schema, desc_db_server, desc_schema):
    ...
    if dds['db_type'] == '9':
        ...
        for d in dres['data']:
            rs2 = await get_ck_query_result(dds, 'select count(0) as rec from `{}`.`{}`'.format(desc_schema, d[1]),
                                            desc_schema)
            try:
                rs1 = await async_processer.query_dict_one_by_ds(sds,
                                                                 'select count(0) as rec from `{}`.`{}`'.format(
                                                                     sour_schema, d[1]))
            except:
                rs1 = {'rec': 0}
...
async def db_stru_compare_data(sour_db_server, sour_schema, desc_db_server, desc_schema):
    ...
    if dds['db_type'] == '9':
        ...
        for d in dres:
            rs1 = await async_processer.query_dict_one_by_ds(dds,
                                                             'select count(0) as rec from `{}`.`{}`'.format(desc_schema,
                                                                                                            d[
                                                                                                                'table_name']))
            try:
                rs2 = await async_processer.query_dict_one_by_ds(sds, 'select count(0) as rec from `{}`.`{}`'.format(
                    sour_schema, d['table_name']))
            except:
                print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
                traceback.print_exc()
                rs2 = {'rec': -1}
