...
jQuery(function($){
  ...
  function read_as_text_with_decoder(file, callback, decoder) {
    ...
    reader.onload = function() {
      ...
      try {
        ...
        if (callback) {
          console.log('read_as_text_with_decoder>>>>',text)
          callback(text);
        }
      }
    };
  ...
  function read_as_text_with_encoding(file, callback, encoding) {
    ...
    reader.onload = function() {
      if (callback) {
        console.log('read_as_text_with_encoding>>>>',reader.result)
  ...
  function connect_without_options() {
    ...
    console.log('connect_without_options->data:',data)
    console.log('connect_without_options->data->hostname:',data.get('hostname'))
    console.log('connect_without_options->url:',url)
    pk = data.get('privatekey');
    ...
    function ajax_post() {
      ...
      console.log('ajax_post=>data=',data)
      console.log('ajax_post=>data->username',data['username'])
      $.ajax({
          url: url,
          type: 'post',
          data: data,
    ...
    if (pk && pk.size && !debug) {
      read_file_as_text(pk, function(text) {
        if (text === undefined) {
          ...
          console.log('before ajax_post....')
          ajax_post();
          console.log('after ajax_post....')
        }
      });
    } else {
      console.log('before ajax_post....2')
      ajax_post();
      console.log('after ajax_post....2')
    }
  ...
  function connect(hostname, port, username, password, privatekey, passphrase, totp) {
    ...
    if (hostname === undefined) {
      console.log('connect->connect_without_options->')
      result = connect_without_options();
      console.log('connect->connect_without_options->result:',result)
    } else {
